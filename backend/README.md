...
The backend deploys to **Render** as a Docker web service; the frontend
deploys to **Netlify** as a static build. Config for both lives at the repo
root: `render.yaml` (Blueprint) and `netlify.toml`.

### Backend on Render

1. In the Render dashboard: **New -> Blueprint**, point it at this repo.
   Render reads `render.yaml` and provisions a web service
   (`studymate-backend`, built from `backend/Dockerfile`) plus a managed
   Postgres instance (`studymate-db`).
2. Set the two secrets the blueprint leaves blank: `OLLAMA_API_KEY` and
   `HUGGINGFACE_TOKEN` (service -> Environment). `JWT_SECRET_KEY` is
   auto-generated by the blueprint.
3. Update `CORS_ALLOWED_ORIGINS` to your actual Netlify origin once you have
   it, e.g. `["https://studymate.netlify.app"]` — the placeholder in
   `render.yaml` won't work as-is.
4. The container's entrypoint (`docker-entrypoint.sh`) runs
   `alembic upgrade head` before starting `uvicorn` on Render's `$PORT`, so
   migrations apply automatically on every deploy — no manual step needed.
5. `DATABASE_URL` is wired automatically from the linked Postgres instance
   (`fromDatabase`); `DATABASE_SSL=true` is set alongside it since Render's
   Postgres requires SSL. Don't touch the discrete `POSTGRES_*` vars —
   `DATABASE_URL` takes priority over them (see `config.py`).
6. pgvector: the `CREATE EXTENSION IF NOT EXISTS vector` in migration
   `0001` needs the extension to be allowed on your Postgres plan. Render
   supports pgvector on their Postgres offering; if the migration fails
   with a permissions error, check the extension is enabled for your
   instance/plan before retrying.

Without a Blueprint, the equivalent manual setup is: a Render Postgres
instance, a Render web service built from `backend/Dockerfile` with the
same env vars as above, and health check path `/health`.

### Frontend on Netlify

1. **New site from Git**, point it at this repo. Netlify reads
   `netlify.toml` at the repo root (`base = "frontend/"`,
   `publish = "dist"`, plus the SPA catch-all redirect so client-side
   routes like `/app/:chatId` don't 404 on refresh).
2. Set `VITE_API_BASE_URL` as a site environment variable to the Render
   backend's URL from the previous step (no trailing slash), e.g.
   `https://studymate-backend.onrender.com`. Vite bakes this in at build
   time — changing it later requires triggering a new deploy.
3. Deploy, then go back and fix `CORS_ALLOWED_ORIGINS` on the Render side to
   this site's real origin (step 3 above) — chicken-and-egg with step 1,
   but both sides need each other's final URL.
